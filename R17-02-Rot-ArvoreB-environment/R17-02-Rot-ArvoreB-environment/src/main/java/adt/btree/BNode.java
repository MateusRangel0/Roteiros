...
public class BNode<T extends Comparable<T>> {
	...
	protected void split(){
		int median = (int) Math.floor(size()/2.0);
		...
		BNode<T> leftNode = generateChildNode(0,  median - 1);
		BNode<T> rightNode = generateChildNode(median + 1, size() - 1);
		
		int pos = parent.indexOfChild(this);
		
		parent.removeChild(this);
		parent.addChild(pos, leftNode);
		parent.addChild(pos + 1, rightNode);
	...
	private BNode<T> generateChildNode(int left, int right) {
		LinkedList<T> elements = new LinkedList<T>();
		LinkedList<T> nodeElements = getElements();

		BNode<T> ret = new BNode<T>(order);
